...
ContentRadar 是一个多平台公开内容采集工具，支持小红书、抖音、快手、B 站、微博、贴吧、知乎等主流内容平台。它不是单个平台的脚本集合，而是把不同平台的搜索、详情、评论、创作者主页、登录态、代理和数据保存方式统一到一套命令入口里。
...
ContentRadar 的思路更偏工程化：先通过 Playwright 打开真实浏览器页面，保存登录态和 Cookie，再在浏览器上下文中执行必要的 JS 表达式，获得平台请求需要的参数。这样可以尽量复用真实页面环境，减少手工逆向复杂加密算法的工作量。
