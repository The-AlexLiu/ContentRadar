...
CDP（Chrome DevTools Protocol）模式是一种基于本地浏览器会话的调试与采集模式，通过控制用户现有的Chrome/Edge浏览器来进行网页爬取。与传统的Playwright自动化相比，CDP模式具有以下优势：

### 🎯 主要优势

1. **真实浏览器环境**: 使用用户实际安装的浏览器，包含所有扩展、插件和个人设置
2. **更接近真实浏览器环境**: 浏览器指纹更加真实，更接近用户日常访问环境
...
| 模式 | 说明 | 适用场景 |
|------|------|----------|
| **连接已有浏览器**（默认推荐） | 连接用户正在使用的 Chrome 浏览器，复用真实的 Cookie、扩展和浏览历史 | 需要复用本地浏览器环境，降低账号异常风险 |
...
这是**默认且推荐**的方式，直接连接你正在使用的 Chrome 浏览器，本地环境一致性最好。

#### 第一步：确保 Chrome 版本

需要 Chrome **144 或更高版本**。在地址栏输入 `chrome://version` 查看当前版本。
...
### 1. 本地调试与稳定性
- 保持 `CDP_HEADLESS = False`，方便观察页面状态和登录流程
- 使用当前浏览器的正常 User-Agent
- 控制请求频率，避免对平台造成压力或触发账号风险
...
两种方式都减少了不同浏览器环境之间的差异，更适合本地调试和低频研究。连接已有浏览器模式的本地环境一致性更好，因为使用的是用户真实的浏览器环境。
